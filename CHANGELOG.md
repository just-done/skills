...
- `find-a-name` skill: proposes names for a business or project, checks the matching domains
  in a single call and returns a link to register the chosen name on justdomain.ai.
- `just-domain` skill: checks specific domains, reports the registration and renewal price
  for the full term, returns a registration link, and checks whether a domain registered
  elsewhere can be transferred to Just Domain.
